...
module.exports = function (io) {
  io.on("connection", async (socket) => {
    ...
    socket.roomId = socket.handshake.headers.referer.split(
      process.env.DOMAIN + "/room/"
    )[1];
    socket.requestInfo = {
      datetime: null,
      requests: 0,
    ...
    socket.emit("user-init", {
      ...
      currentPlayingIndex:
        roomObj.currentPlayingIndex > -1 ? roomObj.currentPlayingIndex : -1,
    ...
    socket.on("playlist-add", async (id) => {
      ...
      await axios
        .get(
          `https://www.googleapis.com/youtube/v3/videos?key=${process.env.GOOGLE_YOUTUBE_API_KEY}&part=snippet&id=${id}`
        )
        .then(({ data }) => {
      ...
      await axios
        .get(
          `https://www.googleapis.com/youtube/v3/videos?key=${process.env.GOOGLE_YOUTUBE_API_KEY}&part=contentDetails&id=${id}`
        )
        .then(({ data }) => {
          if (data.items.length > 0) {
    ...
    socket.on("playlist-change-playing", (i) => {
      ...
      if (!isNaN(index)) {
        ...
        if (
          index > roomObj.playlist.length - 1 ||
          (roomObj.playlist.length > 0 && index < 0)
        ) {
          // Not valid
          socket.emit("socket_invalid_input");
